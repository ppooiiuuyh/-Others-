...
public class ToGate_IMG_TCP implements Runnable {
    ...
    public void run() {
        ...
        final int TYPE_LEN = 4;
        final int DATE_LEN = 128;
        final int DATAOVERVIEW_LEN = 128;
        final int BLANK_LEN = 1024;
        final int TOTAL_LEN_WITHOUT_DATA = TYPE_LEN + DATE_LEN + DATAOVERVIEW_LEN + BLANK_LEN;
        final int DATA_LEN;
        /************************************************************
         * 데이터 구조
         * 처리 타입 [4]
        ...
        try {
             ...
             *헤더 [4]
             ************************************************************/
            finalBytes = makeFormWithString("imag", TYPE_LEN);
...
*/
                ...
                File dest = new File("images\\" +fileName+System.currentTimeMillis()+".jpg");
                ...
                //blank에 데이터 사이즈 전달
                byte[] dataSize = Integer.toString(control).getBytes();
                for(int i=0 ;i<dataSize.length; i++){
                    finalBytes[i+260] = dataSize[i];
                }
